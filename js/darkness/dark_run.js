const DARK_RUN = {
    ...
    mass_glyph_gain: [
        ()=>player.mass.gte(EVO.amt>=3?'ee18':EVO.amt>=1?'ee26':'ee25')?player.mass.log10().div(EVO.amt>=3?'e18':EVO.amt>=1?'e26':'e25').log(1.1).add(1).softcap(50,0.5,0).mul(glyphUpgEff(7)).mul(tmp.dark.glyph_mult).floor():E(0),
        ()=>EVO.amt>=2?player.evo.wh.fabric.add(1).log10().div(5).pow(1.5).mul(tmp.dark.glyph_mult).floor()
			:(player.bh.mass.gte(EVO.amt>=1?'ee18':'e1e18')?player.bh.mass.log10().div(EVO.amt>=1?'e18':1e18).log(1.1).add(1).softcap(50,0.5,0).mul(tmp.dark.glyph_mult).floor():E(0)),
        ()=>player.atom.quarks.gte(EVO.amt>=1?'ee17':'e12')?player.atom.quarks.log10().div(EVO.amt>=1?'e17':1e12).log(1.1).add(1).softcap(50,0.5,0).mul(tmp.dark.glyph_mult).floor():E(0),
        ()=>tmp.atom.unl&&player.md.mass.gte(EVO.amt>=1?'ee9':'ee8')?player.md.mass.log10().div(EVO.amt>=1?'e8':1e21).log(1.1).add(1).softcap(50,0.5,0).mul(tmp.dark.glyph_mult).floor():E(0),
        ()=>tmp.star_unl&&player.stars.points.gte(EVO.amt>=1?'ee11':'e1.5e12')?player.stars.points.log10().div(EVO.amt>=1?'e11':1.5e12).log(1.1).add(1).softcap(50,0.5,0).mul(tmp.dark.glyph_mult).floor():E(0),
        ()=>tmp.prestiges.base.gte(EVO.amt>=3?1e9:EVO.amt>=1?1e10:1e6)?tmp.prestiges.base.div(EVO.amt>=3?1e9:EVO.amt>=1?1e10:1e6).log(1.1).add(1).softcap(10,0.5,0).mul(tmp.dark.glyph_mult).floor():E(0),
    ],
