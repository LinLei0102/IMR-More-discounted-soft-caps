const DARK = {
    ...
    rayEffect() {
        ...
        if (a.gte(1e9)) x.passive = a.div(1e9).max(1).log10().add(1).pow(2).div(50)
    ...
    shadowEff() {
        ...
        x.mass = hasCharger(4) ? overflow(a.add(1),10,0.25) : a.add(1).log10().add(1).root(15)
        ...
        if (a.gte(1e25)) {
			...
			x.en = a.div(1e25).pow(7).overflow('ee10',1/3)
		}
    ...
    abEff() {
        ...
        x.msoftcap = a.add(1).log10().root(hasElement(159)?1:2).div(2).add(1)
        if (a.gte(1e120)) {
...
function updateDarkHTML() {
    ...
    if (tmp.tab_name == "dark-eff") {
        ...
        let e = getNextDarkEffectFromID(1) +`
            Boosts mass gain and mass upgrade 1-3 effect exponent by <b>^${eff.mass.format(3)}</b><br>
        ...
        if (tmp.chal14comp) {
            ...
            e = getNextDarkEffectFromID(2) + `
                ...
                <br>Makes mass gain softcaps 4-5 start <b>^${eff.msoftcap.format(3)}</b> later
            `
