    ...
    upgs: [
        ...
        },
        {
            ...
            effect() {
                ...
                return x.softcap(25,0.1,0)
            },
            effDesc(x) { return"^"+format(x)+softcapHTML(x,'25') },
        },
        {
            desc: `The 3rd challenge's effect is twice as effective.`,
            cost: E(1e10),
        },
        {
            ...
            effect() {
                ...
                if (hasElement(276)) {
                    ...
                    else x =E(1.01).pow(x).overflow(10,0.5).overflow(1e5,0.5)
                }
                return x
            },
            effDesc(x) { return hasElement(276) ? formatPow(x) : formatMult(x)+softcapHTML(x,'10') },
        },
        {
        ...
        {
            desc: `Silicon's effect is +100% better for each element bought.`,
            cost: E(1e25),
            effect() {
                let x = getElemLength(0)
                return Number(x)
            },
        ...
        },
        {
            ...
            cost: E(1e31),
            effect() {
                let x = overflow(hasElement(129) ? player.build.cosmic_ray.amt.pow(0.5).add(1) : player.build.cosmic_ray.amt.pow(0.5).mul(0.01).add(1),1000,0.5)
        ...
        },
        {
            ...
            cost: E(1e36),
            effect() {
                let x = E(1.025).pow(player.build.tickspeed.amt)
                return x
            },
        ...
        },
        {
            ...
            effect() {
                ...
                let x = hasPrestige(0,32) ? player.atom.atomic.max(1).log10().add(1).log10().add(1).root(2) : player.atom.atomic.max(1).log10().add(1)
                return x
            },
            effDesc(x) { return hasPrestige(0,32) ? formatPow(x) : format(x)+"x" },
        },
        {
        ...
        {
            desc: `Super/Hyper Rank & Tickspeed scales 25% weaker.`,
            cost: E(1e58),
        },
        {
        ...
        {
            desc: `8th challenges' scalings are weaker.`,
            cost: E(1e66),
        },
        {
            desc: `Rage power boosts Relativistic particles gain.`,
            cost: E(1e73),
            effect: () => tmp.rp.unl ? player.rp.points.max(1).log10().add(1).pow(2) : E(1),
            effDesc(x) { return format(x)+"x" },
        },
        {
            desc: `Mass from Black Hole boosts dilated mass gain.`,
            cost: E(1e74),
            effect: () => tmp.bh.unl ? player.bh.mass.max(1).log10().add(1).pow(1.3) : E(1),
            effDesc(x) { return format(x)+"x" },
        },
        {
            desc: `Unlock Stars.`,
        ...
        },
        {
            ...
            effect() {
                ...
                let x = tmp.atom.atomicEff[0].add(1).pow(2)
                if (hasElement(82)) x = x.pow(3)
                return x.floor()
            },
        ...
        },
        {
            ...
            cost: E('e153'),
            effect() {
                let x = tmp.star_unl?tmp.stars.effect[0].add(1).pow(0.2):E(1)
                return x
            },
            effDesc(x) { return format(x)+"x" },
        },
        ...
        {
            desc: `Mass Dilation upgrade 4 is 10% stronger.`,
            cost: E('e300'),
        },
        {
        ...
        {
            get desc() { return EVO.amt >= 2 ? `Square Atomic Upgrade 6.` : `Super/Hyper BH Condenser & Cosmic Ray scale 25% weaker,Remove Mass Dilation upgrade 5 1st softcap.` },
            cost: E('e8e3'),
        },
        {
        ...
        },
        {
            ...
            cost: E('e1.2e4'),
            effect() {
                let x = BUILDINGS.eff('tickspeed','power').max(1).log10().max(1)
        ...
        {
            desc: `Hyper Rank & Tickspeed scale weaker based on Tier.`,
            cost: E('e3.4e4'),
            effect() {
                let x = E(0.97).pow(player.ranks.tier)
                return x
            },
            effDesc(x) { return format(E(1).sub(x).mul(100))+"% weaker" },
        },
        {
            get desc() { return EVO.amt >= 2 ? `Gain 10x more Apples.` : `The power from the mass of the BH formula is increased to 1.` },
            cost: E('e5e4'),
        },
        {
        ...
        {
            desc: `Multiply Particle Powers gain by ^0.7 of its Particle's amount after softcap.`,
            cost: E('e97000'),
        },
        {
            desc: `Hyper rank scaling starts 3 later for every supernova.`,
            cost: E('e139000'),
            effect() {
        ...
        {
            get desc() { return EVO.amt >= 2 ? `Increase Wormhole loselessness by 20%.` : `Non-Bonus tickspeeds are 2x more effective.` },
            cost: E('e1.45e5'),
        },
        {
            desc: `Rewards from Challenges 3 are 400% more effective.`,
            cost: E('e1.48e5'),
        },
        {
            desc: `Add 200 more C7 & C8 maximum completions and Reduce C1-C4 goal.`,
            cost: E('e1.58e5'),
        },
        {
        ...
        },
        {
            ...
            effect() {
                if (!tmp.star_unl) return E(1)
                return hasElement(236) || EVO.amt >= 2 ? Decimal.pow(1.1,player.stars.points.add(1).log10().add(1).log10()) : overflow(player.stars.points.add(1).pow(1.5).softcap('e3e15',0.85,2),'ee100',0.5)
            },
        ...
        {
            desc: `Add more C5-6 & C8 maximum completions based on Supernovas and Reduce C1-C4 goal .`,
            cost: E('e1e7'),
            effect() {
                if (!tmp.sn.unl) return E(0)
        ...
        {
            desc: `Pre Meta-Supernova scales 20% weaker.`,
            cost: E('e4e10'),
        },
        {
        ...
        {
            get desc() { return EVO.amt >= 2 ? `Gain 100x more Quantum Foam.` : `Stronger & Tickspeed are 1e25x stronger.` },
            get cost() { return EVO.amt >= 2 ? E('e7e12') : E('e1.4e11') },
        },
        {
            desc: `Stronger is ^1e25 stronger.`,
            cost: E('e1.5e11'),
        },
        {
        ...
        {
            desc: `Pre-Ultra Rank scale weaker based on Cosmic Ray's free tickspeeds.`,
            cost: E('e1e12'),
            effect() {
                if (!tmp.atom.unl) return E(1)
        ...
        },
        {
            ...
            cost: E('e400'),
            effect: () => tmp.bh.unl ? player.bh.mass.add(1).log10().add(1).log10().mul(1.25).add(1).pow(hasElement(201)||player.qu.rip.active?7:1) : E(1),
        ...
        {
            desc: `Entropic Accelerator & Booster nerfing is 10% weaker and Enthalpy gain is better.`,
            cost: E('e1500'),
        },
        {
        ...
        {
            desc: `Prestige Base’s exponent is increased based on Pent and Mass gain softcap^4 is 50% weaker (only 20% in Big Rip).`,
            cost: E('e2.5e7'),
            effect() {
        ...
        {
            desc: `Mass gain softcap^4 is 20% weaker.`,
            cost: E('e9.75e8'),
        },
        {
            ...
            effect() {
                if (!tmp.sn.unl) return E(1)
                return player.supernova.stars.add(1).log10().add(1).log10().add(1)
            },
            effDesc(x) { return formatPow(x) },
        },
        {
        ...
        },
        {
            ...
            get desc() { return EVO.amt >= 2 ? `Untritrium-133 effect in Big Rip works outside of Big Rip.` : `Stronger’s effect and Quark softcap is slightly weaker.` },
            cost: E("e2.75e18"),
        },{
            ...
            br: true,
            desc: `Meta-Rank Boost affects Ultra-Tier starting at a reduced rate.`,
            cost: E("e1.3e19"),
            effect() {
            ...
            desc: `Abyssal Blot’s second effect are 100% booster.`,
            get cost() { return E( EVO.amt >= 1 ? "e8e68" : "e1.9e19") },
        },{
            br: true,
            desc: `Stronger Power’s softcap is weaker and Break dilation upgrade 12 is cheaper.`,
            cost: E("e1.45e19"),
        },{
            dark: true,
            desc: `Unlock Dark Run. Keep Oganesson-118 on darkness(Endgame).`,
            cost: E("e50"),
        },{
