    ...
    particle: {
        ...
        eff: [
            ...
            p=>{
                ...
                let x = [p.root(3).div(5).add(1).softcap(3,0.4,0,br16),p.pow(0.85).add(1)]
            ...
            p=>{
                let x = [p.root(2).div(5), p.root(8).pow(QCs.active()?2:1).overflow(1e11,1/3)]
                return x
            },
            p=>{
            ...
            p=>{
                ...
                let x = hasUpgrade('br',22) ? p.add(1).root(20).softcap(1e30,0.25,0) : p.pow(0.5).softcap(1500,0.5,0)
                return x
            },
        ],
        effDesc: [
            x=>{ return `Boost Stronger Power by ^${format(x)}` },
            ...
            x=>{ return `Boost Higgs Boson's effect by ^${format(x)}` },
