...
const CHALS = {
    ...
    getChalData(x, r=E(-1)) {
        ...
        if (x > 16) {
            ...
            if (hasElement(65) && (x==1||x==2||x==3||x==4)) pow = pow.mul(0.85)   
            if (hasElement(151) && (x==13||x==14)) pow = pow.mul(0.6)        
            if (hasElement(73) && (x==1||x==2||x==3||x==4)) pow = pow.mul(0.85)         
                if (hasElement(33) && (x==8)) pow = pow.mul(0.8)     
    ...
    1: {
        unl() { return EVO.amt < 2 && (player.mass.gte(1e120) || player.chal.unl || player.atom.unl) },
        title: "Instant Scale",
        desc: "Super Ranks start at 10 and Super Tickspeed starts at 5.",
    ...
    2: {
        ...
        reward: `+100% Tickspeed Power per completion.`,
        max: E(100),
        inc: E(10),
        pow: E(1.3),
        start: E(1e27),
        effect(x) {
            ...
            let ret = x.add(1).softcap(10,sp,0).sub(1)
            return ret
        },
        effDesc(x) { return "+"+format(x.mul(100))+"%"+(x.gte(10)?" <span class='soft'>(softcapped)</span>":"") },
    },
    3: {
        unl() { return EVO.amt < 2 && (player.chal.comps[2].gte(1) || player.atom.unl) },
        title: "Melted Mass",
        desc: "Mass gain softcap is stronger.",
        ...
        start: E(1e12),
        effect(x) {
            if (hasElement(64)) x = x.mul(5)
            if (hasElement(5)) x = x.mul(2)
            let ret = hasElement(133) ? x.root(4/3).add(1) : x.root(3).mul(0.025).add(1)
    ...
    4: {
        ...
        title: "Weakened Rage",
        get desc() { return `Reduce Rage Powers by ^^0.5.` },
    ...
    6: {
        ...
        reward: () => EVO.amt >= 2 ? `Gain +100% more Fabric per completion.` : `Every completion adds 100% to tickspeed and BH condenser power.`,
        max: E(50),
        inc: E(64),
        pow: E(1.25),
        start: E(1.989e38),
        effect(x) {
            if (EVO.amt < 2) return x.add(1).softcap(5,hasElement(39)?1:0.5,0).sub(1)
            if (EVO.amt >= 2) return x.mul(hasElement(80,1) ? E(1.1).pow(x) : 1).add(1)
        },
        effDesc(x) { return EVO.amt>=2?formatMult(x):"+"+format(x)+"x"+(x.gte(5)?" <span class='soft'>(softcapped)</span>":"") },
    },
    7: {
    ...
    10: {
        ...
        inc: E('e500'),
        pow: E(2),
        start: E('e4000').mul(1.5e56),
        effect(x) {
            let ret = EVO.amt >= 3 ? Decimal.pow(1.1,expMult(x,0.5)).softcap(1e9,3,'log') : x.pow(1.75).mul(EVO.amt >= 2 ? 0.1 : 0.025).add(1)
            return ret
        },
    ...
    12: {
        ...
        inc: E('ee5'),
        pow: E(2),
        get start() { return EVO.amt >= 2 ? uni('ee5') : uni('e5e6') },
        effect(x) {
            let ret = x.pow(hasTree("chal7a")?1.5:1.25)
