const MASS_DILATION = {
    ...
    penalty() {
        ...
        if (QCs.active() && (brokeDil() ? !player.qu.rip.active : true)) x **= tmp.qu.qc.eff[6]
        return x
    },
    onactive() {
    ...
    RPexpgain() {
        ...
        if (QCs.active()) x = x.mul(tmp.qu.qc.eff[4])
        if (hasElement(24) && hasPrestige(0,32)) x = x.mul(tmp.elements.effect[24])
    ...
    massGain() {
        if (CHALS.inChal(11)) return E(0)
        let pow = E(2).add(mdEff(1, true, 0))
        let x = player.md.particles.pow(pow)
        x = x.mul(mdEff(0))
        if (hasElement(22)) x = x.mul(elemEffect(22))
        if (hasElement(35)) x = x.mul(elemEffect(22))
        if (hasElement(40)) x = x.mul(elemEffect(40, [1, 1])[0])
        if (player.ranks.rank.gte(615)) x = x.mul(RANKS.effect.rank[72]())     
        if (hasElement(32)) x = x.pow(1.05)
        if (QCs.active()) x = x.pow(tmp.qu.qc.eff[4])

        x = x.softcap(tmp.md.massSoftcap1,0.5,0)

        if (hasElement(40)) x = x.pow(elemEffect(40, [1, 1])[1])
        ...
        let o = x
        let os = E('1.798e308').pow(glyphUpgEff(8))
    ...
    effect() {
        let x = tmp.md.bd3 ? player.qu.rip.active ? player.md.mass.max(1).log10().max(1).log10().add(1).log10().add(1).root(2) : player.md.mass.max(1).log10().max(1).log10().add(1).root(3) : player.md.mass.max(1).log10().add(1)
        x = x.mul(mdEff(1))
        return x
    },
    upgs: {
        ...
        ids: [
            {
				...
				effect(x) {
					let r = player.md.mass.max(1).log(100).root(25).div(25).add(1)
                    if (hasElement(53)) r = r.pow(1.1) 
                ...
                effect(x) { return E(0.9).pow(x.softcap(15,hasElement(55)?1:0.5,0).softcap(25,0.2,0)) },
                effDesc(x) { return format(E(1).sub(x).mul(100))+"% weaker"+(x.lt(0.2) && EVO.amt < 2?" <span class='soft'>(softcapped)</span>":"") },
            },{
                ...
                maxLvl: 1,
                cost(x) { return E('1.5e311') },
                bulk() { return player.md.mass.gte('1.5e311')?E(1):E(0) },
                effect(x) { return E(2).pow(player.md.mass.max(1).log10()).overflow(10,0.5) },
                effDesc(x) { return format(x)+"x" },
            },{
                desc: `Mass Dilation upgrade 2 effect's formula is better.`,
                maxLvl: 1,
                cost(x) { return E('1.5e321') },
                bulk() { return player.md.mass.gte('1.5e321')?E(1):E(0) },
            },{
                ...
                maxLvl: 1,
                cost(x) { return E('1.5e363') },
                bulk() { return player.md.mass.gte('1.5e363')?E(1):E(0) },
                ...
                cost(x) { return E(5).pow(x).mul('1.50001e370') },
                bulk() { return player.md.mass.gte('1.50001e370')?player.md.mass.div('1.50001e370').max(1).log(5).add(1).floor():E(0) },
                effect(x) {
                    let r = E(2).pow(x)
                ...
                cost(x) { return E(1e50).pow(x.pow(1.5)).mul('1.50001e486') },
                bulk() { return player.md.mass.gte('1.50001e486')?player.md.mass.div('1.50001e486').max(1).log(1e50).max(0).root(1.5).add(1).floor():E(0) },
                effect(i) {
                ...
                cost(x) { return E(1e100).pow(x.pow(2)).mul('1.5e656') },
                bulk() { return player.md.mass.gte('1.5e656')?player.md.mass.div('1.5e656').max(1).log(1e100).max(0).root(2).add(1).floor():E(0) },
                effect(x) {
    ...
    break: {
        ...
        energyGain() {
            if (!brokeDil()) return E(0)
            if (!hasElement(136) && !player.qu.rip.active) return E(0)

            let x = player.md.mass.add(1).log10().sub(125).max(0)
            let p = x.add(1).log10()
            if (hasElement(127)) p = p.mul(1.1)

            x = x.add(1).pow(p).sub(1)
            if (hasPrestige(0,10)) x = x.mul(prestigeEff(0,10))
            x = x.mul(mdEff(5, true))
            if (hasElement(116)) x = x.mul(elemEffect(116))

            return x
        },
        massGain() {
            let x = player.md.break.energy.max(0).pow(2)
            x = x.mul(mdEff(0, true))
            if (hasMDUpg(7, true)) x = x.mul(mdEff(7, true))
            if (hasMDUpg(8, true)) x = x.mul(mdEff(8, true))
            ...
            ids: [
                {
                    ...
                    maxLvl: 1,
                    cost(x) { return uni(1e60) },
                    bulk() { return player.md.break.mass.gte(uni(1e60))?E(1):E(0) },
                    effect(y) {
                        let x = (tmp.qu.speed||E(1)).add(1).root(10)

                        return x
                    },
                    effDesc(x) { return format(x)+"x" },
                },{
                    desc: `Super Prestige starts 10 later.`,
                    maxLvl: 1,
                    cost(x) { return uni(1e65) },
                    bulk() { return player.md.break.mass.gte(uni(1e65))?E(1):E(0) },
                },{
                    unl: ()=>player.dark.unl,
                    desc: `You can now automatically evaporate resources. (Stronger than manual).`,
                    maxLvl: 1,
                    cost(x) { return uni(1e110) },
                    bulk() { return player.md.break.mass.gte(uni(1e110))?E(1):E(0) },
                },{
                    unl: ()=>player.dark.unl,
                    desc: `Double dark shadows gain.`,
                    cost(x) {
                        x = x.scale(17,hasElement(160)?1.5:3,0)
                    ...
                    bulk() {
                        ...
                        y = y.scale(17,hasElement(160)?1.5:3,0,true)
...
function updateMDTemp() {
	...
	tmp.md.upgs = []
    for (let x = MASS_DILATION.upgs.ids.length - 1; x >= 0; x--) {
...
function updateBDTemp() {
    let bd = tmp.bd = {}
	bd.upgs = []
    for (let x = MASS_DILATION.break.upgs.ids.length - 1; x >= 0; x--) {
...
function updateMDHTML() {
    tmp.el.md_particles.setTxt(format(player.md.particles,0)+(hasTree("qol3")?" "+formatGain(player.md.particles,tmp.md.passive_rp_gain.mul(tmp.qu.speed)):""))
    ...
    tmp.el.md_mass.setTxt(formatMass(player.md.mass)+" "+formatGain(player.md.mass,tmp.md.mass_gain.mul(tmp.qu.speed),true))
    tmp.el.md_btn.setTxt(inMD()
    ...
    for (let x = 0; x < MASS_DILATION.upgs.ids.length; x++) {
        ...
        if (unl) {
            let has = player.md.upgs[x].gte(upg.maxLvl||1/0)
            tmp.el["md_upg"+x+"_div"].setClasses({btn: true, full: true, md: true, bought: has, locked: !has && !tmp.md.upgs[x].can })
...
function updateBDHTML() {
    ...
    tmp.el.bd_btn.setDisplay(!bd.active)
    ...
    for (let x = 0; x < MASS_DILATION.break.upgs.ids.length; x++) {
        ...
        if (unl) {
            let has = player.md.break.upgs[x].gte(upg.maxLvl||1/0)
            tmp.el["bd_upg"+x+"_div"].setClasses({btn: true, full: true, bd: true, bought: has, locked: !has && !tmp.bd.upgs[x].can, corrupted_text2: x == 2 && c16 })
...
//has, eff
function hasMDUpg(x, bd) {
	return (bd ? player.md?.break : player.md)?.upgs[x].gt(0)
}

function mdEff(x, bd, def = E(1)) {
	return tmp[bd ? "bd" : "md"]?.upgs?.[x].eff ?? def
}

function inMD() {
	return player.md?.active
}
