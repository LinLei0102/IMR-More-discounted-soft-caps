const RANKS = {
    ...
    desc: {
        rank: {
            ...
            '4': "reduce mass upgrade 3 scaling by 20%. Ranks boost mass by (x+1)^x.",
            '5': "mass upgrade 2 boosts itself.",
            '7': "reduce all mass upgrade scaling by 15%.",
            '10': "mass upgrade 3 boosts itself.",
            '13': "1000x mass gain.",
            '17': "Rank 4 reward effect is better. [^x → ^x^1.1]",
            '34': "mass upgrade 3 softcaps 1.2x later.",
            '40': "muls tickspeed power based on ranks.",
            '45': "rank boosts Rage Powers gain.",
            '72': "rank multiplies mass gain.",
            '78': "reduce rank requirements by 10%.",
            '83': "rank 10 and rank 40 reward is stronger.",
            '86': "C2 reward is Hole Conderser but weeker.",
            '180': "mass gain is raised by 1.05.",
            '220': "rank 40 reward is overpowered.",
            '230': "rank multiplies quark gain.",
            '233': "mass upgrade 1 boosts itself in power.",
            '360': "mass upgrade 2 boosts itself in power.",
            '450': "make mass gain softcap 0.25% weaker based on rank, hardcaps at 90%.",
            '480': "Gluon Upgrade 4's effect is better.",
            '615': "rank 17 reward is overpowered and rank 72 multiplies dilated mass gain",
            '1100': "Remove all softcaps from Gluon Upgrade 4's effect",
        },
        tier: {
            '1': "reduce rank requirements by 20%. Ranks can be automated.",
            '2': "raise mass by ^1.15",
            '3': "reduce all mass upgrade scalings by 25%.",
            '4': "x2 tickspeed power for every tier you have, softcaps at x40.",
            ...
            '55': "make rank 72's effect stronger based on tier.",
            '100': "Super Tetr scales 5 later.",
            '160': "mass upgrade 3 boosts itself in power.",
        },
        tetr: {
        ...
        pent: {
            '1': "reduce tetr requirements by 15%, and Ultra-Rank starts 1.1x later.",
        ...
        hex: {
            ...
            '4': "increase dark ray gain by x150% per hex.",
    ...
    effect: {
        rank: {
            ...
            '4'() {
				let b = 1, e = player.ranks.rank.add(1)
				if (hasUpgrade("rp", 4)) e=e.pow(1.05)
				if (player.ranks.rank.gte(17)) e = player.ranks.rank.add(1).pow(1.1)
                if (player.ranks.rank.gte(615)) e = player.ranks.rank.add(1).pow(player.ranks.rank.add(10).log10().pow(0.9))
				return player.ranks.rank.div(b).add(1).pow(e)
            },
            '5'() {
            ...
            '10'() {
                ...
                if (player.ranks.rank.gte(83)) ret = ret.pow(2)
                if (ret.gte(1) && hasPrestige(0,15)) ret = ret.pow(1.5)
                return ret
            },
            '40'() {
                let ret = player.ranks.rank.root(2)
                if (player.ranks.rank.gte(220)) ret = player.ranks.rank.div(100)
                return ret
            },
            '45'() {
                let ret = player.ranks.rank.add(1).pow(3)
                if (player.ranks.rank.gte(83)) ret = player.ranks.rank.add(1).pow(5)
                return ret
            },
            '72'() {
                let ret = EVO.amt>=4 ? E(1) : E(10).pow(player.ranks.rank.pow(1.05).pow(player.ranks.tier.gte(55)?RANKS.effect.tier[55]():1).softcap(1e10,0.1,0))
                return ret
            },
            '230'() {
                let ret = player.ranks.rank.add(1).pow(player.ranks.rank).pow(0.01)
                return ret
            },
            '233'() {
                let ret = player.build.mass_1.amt.add(1).pow(player.prestiges[0].gte(45)?0.6:0.03)
                return ret
            },
            '360'() {
                let ret = player.build.mass_1.amt.add(1).pow(player.prestiges[0].gte(45)?0.8:0.05)
                return ret
            },
            '450'() {
                let ret = E(1).sub(player.ranks.rank.sub(449).mul(0.0035).add(1).softcap(1.25,0.5,0).sub(1)).max(0.1)
                return ret
            },
        },
        tier: {
            '4'() {
                let ret = E(0)
                if (player.ranks.tier.gte(12)) ret =  E(2).pow(player.ranks.tier).overflow(40,0.5).mul(3)
                else ret = E(2).pow(player.ranks.tier).overflow(40,0.5)
                return ret
            },
            '6'() {
            ...
            '160'() {
                let ret = player.build.mass_1.amt.add(1).pow(player.prestiges[0].gte(45)?2:0.175)
                return ret
            },
        },
        tetr: {
            ...
            '4'() {
                let ret = E(0.96).pow(player.ranks.tier)
                return ret
            },
            '5'() {
    ...
    effDesc: {
        rank: {
            ...
            40(x) {  return format(x)+"x" },
            45(x) { return format(x)+"x" },
            72(x) { return format(x)+"x" },
            230(x) { return format(x)+"x" },
            233(x) { return "^"+format(x) },
            360(x) { return "^"+format(x) },
            450(x) { return format(E(1).sub(x).mul(100))+"% weaker" },
        },
        tier: {
    ...
    fp: {
        rank() {
            ...
            if (player.ranks.rank.gte(78)) f = f.mul(1/0.85)    
...
const PRESTIGES = {
    ...
    baseExponent() {
        ...
        if (hasPrestige(0,28)) x = x.add(prestigeEff(0,32,0))
    ...
    req(i) {
        ...
        switch (i) {
            case 0:
                x = Decimal.pow(1.1,y.scaleEvery('prestige0',false,[0,0,0,fp]).pow(1.1)).mul(EVO.amt>=2?2e12:2e9)
                break;
            case 1:
    ...
    bulk(i) {
        ...
        switch (i) {
            case 0:
                if (y.gte(EVO.amt>=2?2e12:2e9)) x = y.div(EVO.amt>=2?2e12:2e9).max(1).log(1.1).max(0).root(1.1).scaleEvery('prestige0',true,[0,0,0,fp]).add(1)
                break;
            case 1:
    ...
    rewards: [
        {
            ...
            "28": `Prestige Base’s exponent is increased based on Prestige.`,
            "32": `Chromium-24 is slightly stronger.`,
            "45": `Rank 233,360 and Tier 160 is slightly stronger.`,
...
function updateRanksTemp() {
    ...
    fp = E(1).mul(ffp)
    let pow = 2
    if (hasElement(44)) fp = fp.mul(1/0.65)
    ...
    tmp.ranks.pent.req = player.ranks.pent.div(ifp).div(ffp2).scaleEvery('pent',false,fpa).div(fp).pow(pow).add(9-tps).floor()
    tmp.ranks.pent.bulk = player.ranks.tetr.sub(9-tps).gte(0)?player.ranks.tetr.sub(9-tps).max(0).root(pow).mul(fp).scaleEvery('pent',true,fpa).mul(ffp2).mul(ifp).add(1).floor():E(0);
