const ATOM = {
    gain() {
        ...
        if (tmp.dark.run) x = expMult(x,mgEff(2))
            if (x.gte('1ee30')) x = expMult(x,0.4,('1ee30'))
        return x.floor()
    },
    quarkGain() {
		let x = tmp.atom.gain
        ...
        if (player.ranks.rank.gte(230)) x = x.mul(RANKS.effect.rank[230]())
        ...
        if (x.gte('1e2000000')) x = expMult(x,hasElement(149)?0.51:0.5,('1e2000000'))
        if (x.gte('1ee19')) x = expMult(expMult(x,0.2,('1ee19')),0.2,('1ee19'))     
        return x.floor()
    },
    ...
    atomic: {
        gain() {
            ...
            if (x.gte('1ee14')) x = expMult(x,0.25,('1ee14'))
            return x
        },
        effect() {
    ...
    particles: {
        ...
        effect(i) {
            ...
            if (hasElement(61)) x = x.mul(p.add(1).pow(0.7))
        ...
        powerEffect: [
            x=>{
                ...
                let b = hasElement(29) ? x.add(1).log2().pow(2.5) : x.add(1).pow(2.5).log2().mul(0.1)
            ...
            x=>{
                ...
                :(hasElement(19)
                ?player.mass.max(1).log10().add(1).pow(bp.max(1).log10().mul(x.max(1).log10()).root(2.25))
                :player.mass.max(1).log10().add(1).pow(bp.max(1).log(100).mul(x.max(1).log(100)).root(3)))
            ...
            x=>{
                ...
                let b = hasElement(30) ? x.add(1).log2().pow(1.2).mul(0.1) : x.add(1).pow(2).log2().mul(0.1)
