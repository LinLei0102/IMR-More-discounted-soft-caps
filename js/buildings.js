...
const BUILDINGS_DATA = {
    mass_1: {
        ...
        effect(x) {
            ...
            if (player.ranks.rank.gte(233)) power = power.pow(RANKS.effect.rank[233]())
            let effect = power.mul(x)
            if (hasElement(209)) effect = effect.pow(elemEffect(209))
                effect = expMult(effect,(tmp.dark.shadowEff.mass))
            if (effect.gte('1ee25')) effect = expMult(expMult(effect,0.5,("1ee25")),0.5,("1ee25"))
            return {power, effect}
        },
        ...
        get_effect: x => "+"+formatMass(x.effect)+" to mass gain"+softcapHTML(x.effect,'1ee25'),
    },
    mass_2: {
        name: "Booster",
		icon: "mass_upg2",
        ...
        effect(x) {
            ...
            if (player.ranks.rank.gte(360)) step = step.pow(RANKS.effect.rank[360]())
            let ret = step.mul(x).add(1)
            if (hasElement(203)) ret = ret.pow(elemEffect(203))
                ret = expMult(ret,(tmp.dark.shadowEff.mass))
            if (ret.gte('5ee25')) ret = expMult(expMult(ret,0.2,("5ee25")),0.2,("5ee25"))
        ...
        get_effect: x => formatMult(x.effect)+" to Muscler Power"+softcapHTML(x.effect,'5ee25'),
    },
    mass_3: {
        name: "Stronger",
		icon: "mass_upg3",
        ...
        effect(x) {
            ...
            if (hasElement(81)) x = x.pow(1e25)
			if (EVO.amt < 2 && hasElement(80)) x = x.mul(hasElement(80)?1e25:1)
            ...
            if (hasUpgrade("rp",9)) step = step.pow(1.25)
            if (hasUpgrade("rp",12)) step = step.pow(upgEffect(1,12,0))
            if (EVO.amt >= 1) step = step.mul(tmp.evo.meditation_eff.mass3??1)
            if (hasElement(4)) step = step.pow(elemEffect(4))
            if (hasMDUpg(3)) step = step.pow(mdEff(3))
			step = step.mul(nebulaEff("red"))
            step = step.pow(BUILDINGS.eff('mass_4'))
            if (player.ranks.tier.gte(160)) step = step.pow(RANKS.effect.tier[160]())
            let sp = 0.1
            if (hasUpgrade("atom",9)) sp *= 1.1
            ...
            ret = ret.pow(tmp.qu.prim.eff[0])
			if (QCs.active() && EVO.amt >= 4) ret = ret.mul(tmp.qu.qc.eff[9])
            if (!player.ranks.pent.gte(15)) ret = ret.softcap(ss2,sp2,0)
                ret = expMult(ret,(tmp.dark.shadowEff.mass))
            let o = ret
            if (o.gte('1e500')) o = expMult(o,0.75,("1e500"))
            let os = E(EVO.amt >= 2 ? 'e70' : '25000').mul(getFragmentEffect('mass')), os2 = E(EVO.amt >= 2 ? 'e600' : '9e15')
            let op = E(0.25), op2 = E(0.01)
        ...
        get_power: x => "+^"+format(x.power)+softcapHTML(x.power,'1e500'),
    ...
    tickspeed: {
        ...
        effect(x) {
            ...
            if (player.ranks.tier.gte(4)) step = step.mul(RANKS.effect.tier[4]())
            if (player.ranks.rank.gte(40)) step = step.mul(RANKS.effect.rank[40]())
            ...
            eff = step.pow(t.mul(hasElement(80)?1e25:1))

            if (!hasElement(199) || CHALS.inChal(15)) {
                if (hasElement(18)) eff = eff.pow(elemEffect(18))
                if (player.ranks.tetr.gte(3)) eff = eff.pow(1.05)
                if (hasElement(150)) eff = expMult(eff,4.5)
            }
            if (eff.gte('1e2.5e10')) eff = expMult(eff,0.1,('1e2.5e10'))
                if (eff.gte('1e2.5e36')) eff = expMult(eff,0.1,('1e2.5e36'))     
            eff_bottom = eff
			if (hasElement(199) && !CHALS.inChal(15)){
        ...
        get_effect: x => (hasElement(199) && !CHALS.inChal(15) ? formatPow(x.effect) : formatMult(x.effect)) + " to mass gain"+softcapHTML(x.effect,'1e2e10'),
    },
    accelerator: {
        name: "Accelerator",
		icon: "accelerator",
    ...
    bhc: {
        ...
        effect(x) {
            ...
            if (player.ranks.rank.gte(86))pow = pow.add(tmp.chal.eff[2].pow(0.55))
    ...
    cosmic_ray: {
        ...
        effect(x) {
            let t = x
            t = t.pow(radBoostEff(10))
            let pow = E(2)
            if (hasUpgrade("atom",4)) pow = pow.add(upgEffect(3,4,0))
            if (hasUpgrade("atom",11)) pow = pow.mul(upgEffect(3,11))
...
function getMassUpgradeCost(i, lvl) {
    ...
    if (i==4) {
        ...
        if (player.ranks.tier.gte(3)) inc = inc.pow(0.75)
        if (player.ranks.rank.gte(7)) inc = inc.pow(0.85)
        if (hasUpgrade("rp",8)) inc = inc.pow(upgEffect(1,8))
