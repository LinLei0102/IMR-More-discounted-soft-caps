...
function getScalingStart(type, name) {
	...
	if (type==0) {
		if (name=="rank") {
			if (CHALS.inChal(1) || CHALS.inChal(10)) return E(10)
			start = start.add(tmp.chal?tmp.chal.eff[1].rank:0)
			if (hasUpgrade("atom",5)) start = start.add(15)
		}  else if (name=="tetr") {
			...
			if (CHALS.inChal(1) || CHALS.inChal(10)) return E(5)
...
function getScalingPower(type, name) {
	...
	let power = E(1)
	if (name == "supernova" && (hasCharger(3)?type<5:type<1)) power = power.mul(fermEff(1, 4))
		if (name == "supernova" && hasElement(78)&&type<3) power = power.mul(0.8)
	if (name == "fTier" && type<3 && hasTree("fn12")) power = power.mul(0.9)
	if (name == "rank" && type<2 && hasElement(84)) power = power.mul(elemEffect(84))
