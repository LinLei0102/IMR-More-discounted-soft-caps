const BOSONS = {
    ...
    effect: {
        pos_w(x) {
            ...
            if (hasTree("qu2") && !player.qu.rip.active) a = a.pow(x.add(1).log10().add(1).pow(2).softcap(1e8,0.01,0))
        ...
        z_boson(x) {
            ...
            if (hasTree("sn4")) a = a.pow(1.5)
            if (hasTree("sn4")) b = b.pow(1.5)
            return [a,b]
        },
        graviton(x) {
        ...
        photon: [
            {
                ...
                effect(i) {
                    let x
                    if (hasElement(213)) {
                        x = player.supernova.bosons.photon.add(1).log10().add(10).log10().mul(i.add(10).log10()).pow(3)
                        if (hasElement(23,1)) x = x.pow(fermEff(0, 3).log10().add(1).root(2))
                    } else {
                        x = player.supernova.bosons.photon.add(1).log10().add(1).pow(i.softcap(8000,0.1,0).pow(fermEff(0, 3)).mul(2)).softcap("ee11",0.8,2).softcap("e4e14",hasElement(99)?0.785:0.75,2)
        ...
        gluon: [
            {
                ...
                effect(x) {
                    ...
                    if (!player.ranks.rank.gte(1100)) y = y.softcap(5.5,0.25,0).softcap(10,0.25,0)
                    if (player.ranks.rank.gte(480)) y = y.pow(2)
                    if (player.ranks.rank.gte(1100)) y = y.pow(x.pow(0.65))    
                    return y
                },
                effDesc(x) { return "/"+format(x)+(x.gte(5.5)&&!player.ranks.rank.gte(1100)?" <span class='soft'>(softcapped)</span>":"") },
            },{
