const TOOLTIP_RES = {
    ...
    md: {
        ...
        desc() {
            ...
            h += `
            <br class='line'><i>
            ${inMD()?`Reach <b>${formatMass(tmp.md.mass_req)}</b> of normal mass to gain Relativistic Particles, or cancel dilation.`:"Dilate mass, then cancel."}<br><br>Dilating mass will force an atom reset. While mass is dilated, all pre-atom resources and atomic power gain will get their multipliers' exponents raised to 0.975<br>
            </i>`
    ...
    qu: {
        ...
        desc() {
            let h = `<i>
            Reach over <b>${formatMass(mlt(EVO.amt>=4 ? 1e3 : 1))}</b> of normal mass to ${QCs.active()?"complete Quantum Challenge":"go Quantum"}.
            </i>`
